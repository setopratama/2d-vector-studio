...
- 🏷️ **Pembersihan Nama File Bersih (Format Spasi Alami Tanpa Underscore `_`)**: Menghilangkan seluruh karakter underscore `_` dan pola `1:1`/`1x1` dari nama file unduhan, menghasilkan format spasi alami yang rapi, mudah dibaca langsung di File Explorer/Finder, dan optimal untuk SEO microstock (misal: `vintage coffee badge isolated on white background.png`).
